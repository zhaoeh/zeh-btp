package jp.co.futech.gateway.filter.grey;
...
@AllArgsConstructor
@Slf4j
@SuppressWarnings({"JavadocReference", "rawtypes", "unchecked"})
public class GrayReactiveLoadBalancerClientFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String localService = environment.getProperty(serviceId);
        if(StringUtils.hasText(localService)){
            String query = url.getRawQuery();
            exchange.getAttributes().put(GATEWAY_REQUEST_URL_ATTR,
                    UriComponentsBuilder.fromUriString("http://" + localService + url.getPath()).query(query).build(true).toUri());
            return chain.filter(exchange);
        }
