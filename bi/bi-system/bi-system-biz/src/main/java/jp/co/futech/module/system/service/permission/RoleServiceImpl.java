...
import jp.co.futech.module.system.controller.admin.permission.vo.role.RolePageReqVO;
...
import jp.co.futech.module.system.dal.mysql.permission.RoleMapper;
...
public class RoleServiceImpl implements RoleService {
    ...
    void validateRoleDuplicate(String name, String code, Long id) {
        ...
        if (RoleCodeEnum.isSuperAdmin(code)) {
            throw exception(ROLE_ADMIN_CODE_ERROR.setParams(Map.of("code", code)), code);
        }
        // 1. 该 name 名字被其它角色所使用
        RoleDO role = roleMapper.selectByName(name);
        if (role != null && !role.getId().equals(id)) {
            throw exception(ROLE_NAME_DUPLICATE.setParams(Map.of("name", name)), name);
        }
        // 2. 是否存在相同编码的角色
        ...
        if (role != null && !role.getId().equals(id)) {
            throw exception(ROLE_CODE_DUPLICATE.setParams(Map.of("code", code)), code);
        }
    }
    ...
    public void validateRoleList(Collection<Long> ids) {
        ...
        ids.forEach(id -> {
            ...
            if (!CommonStatusEnum.ENABLE.getStatus().equals(role.getStatus())) {
                throw exception(ROLE_IS_DISABLE.setParams(Map.of("name", role.getName())), role.getName());
            }
        });
    }
