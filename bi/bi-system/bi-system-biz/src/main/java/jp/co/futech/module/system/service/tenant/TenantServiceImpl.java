package jp.co.futech.module.system.service.tenant;
...
import jp.co.futech.module.system.controller.admin.permission.vo.role.RoleSaveReqVO;
import jp.co.futech.module.system.controller.admin.tenant.vo.tenant.TenantPageReqVO;
import jp.co.futech.module.system.controller.admin.tenant.vo.tenant.TenantSaveReqVO;
import jp.co.futech.module.system.convert.tenant.TenantConvert;
...
import jp.co.futech.module.system.dal.dataobject.tenant.TenantPackageDO;
import jp.co.futech.module.system.dal.mysql.tenant.TenantMapper;
import jp.co.futech.module.system.enums.permission.RoleCodeEnum;
import jp.co.futech.module.system.enums.permission.RoleTypeEnum;
...
import jp.co.futech.module.system.service.tenant.handler.TenantInfoHandler;
import jp.co.futech.module.system.service.tenant.handler.TenantMenuHandler;
...
public class TenantServiceImpl implements TenantService {
    ...
    @Autowired(required = false) // 由于 bi.tenant.enable 配置项，可以关闭多租户的功能，所以这里只能不强制注入
    ...
    public void validTenant(Long id) {
        ...
        if (tenant.getStatus().equals(CommonStatusEnum.DISABLE.getStatus())) {
            throw exception(TENANT_DISABLE.setParams(Map.of("name", tenant.getName())), tenant.getName());
        }
        if (DateUtils.isExpired(tenant.getExpireTime())) {
            throw exception(TENANT_EXPIRE.setParams(Map.of("name", tenant.getName())), tenant.getName());
        }
    }
    ...
    private void validTenantNameDuplicate(String name, Long id) {
        ...
        if (id == null) {
            throw exception(TENANT_NAME_DUPLICATE.setParams(Map.of("name", name)), name);
        }
    ...
    private void validTenantWebsiteDuplicate(String website, Long id) {
        ...
        // 如果 id 为空，说明不用比较是否为相同名字的租户
        if (id == null) {
            throw exception(TENANT_WEBSITE_DUPLICATE.setParams(Map.of("website",website)), website);
        }
        if (!tenant.getId().equals(id)) {
            throw exception(TENANT_WEBSITE_DUPLICATE.setParams(Map.of("website",website)), website);
        }
