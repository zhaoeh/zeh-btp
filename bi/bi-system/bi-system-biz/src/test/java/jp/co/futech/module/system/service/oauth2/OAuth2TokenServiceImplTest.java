package jp.co.futech.module.system.service.oauth2;
...
import jp.co.futech.framework.test.core.ut.BaseDbAndRedisUnitTest;
import jp.co.futech.module.system.controller.admin.oauth2.vo.token.OAuth2AccessTokenPageReqVO;
...
import jp.co.futech.module.system.dal.mysql.oauth2.OAuth2AccessTokenMapper;
import jp.co.futech.module.system.dal.mysql.oauth2.OAuth2RefreshTokenMapper;
import jp.co.futech.module.system.dal.redis.oauth2.OAuth2AccessTokenRedisDAO;
...
public class OAuth2TokenServiceImplTest extends BaseDbAndRedisUnitTest {
    ...
    public void testRefreshAccessToken_null() {
        ...
        assertServiceException(() -> oauth2TokenService.refreshAccessToken(refreshToken, clientId),
                new ErrorCode(400, "无效的刷新令牌", null));
    }
    ...
    public void testRefreshAccessToken_clientIdError() {
        ...
        // 调用，并断言
        assertServiceException(() -> oauth2TokenService.refreshAccessToken(refreshToken, clientId),
                new ErrorCode(400, "刷新令牌的客户端编号不正确", null));
    }
    ...
    public void testRefreshAccessToken_expired() {
        ...
        // 调用，并断言
        assertServiceException(() -> oauth2TokenService.refreshAccessToken(refreshToken, clientId),
                new ErrorCode(401, "刷新令牌已过期", null));
    ...
    public void testCheckAccessToken_null() {
        ...
        assertServiceException(() -> oauth2TokenService.checkAccessToken(randomString()),
                new ErrorCode(401, "访问令牌不存在", null));
    }
    ...
    public void testCheckAccessToken_expired() {
        ...
        assertServiceException(() -> oauth2TokenService.checkAccessToken(accessToken),
                new ErrorCode(401, "访问令牌已过期", null));
    }
