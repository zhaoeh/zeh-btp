package zeh.btp.mfa.aop;

import zeh.btp.mfa.MfaProcess;
import zeh.btp.mfa.annotation.MfaAuth;
import zeh.btp.mfa.component.GoogleAuthHelper;
import zeh.btp.mfa.exception.MfaException;
import zeh.btp.mfa.exception.MfaExceptionThrower;
import zeh.btp.mfa.show.MfaContext;
...
public class MfaCheckAround {
    ...
    public MfaCheckAround(ObjectProvider<MfaProcess> objectProvider, GoogleAuthHelper googleAuthHelper) {
        ...
        MfaExceptionThrower.notNull(process, googleAuthHelper, MfaErrorEnum.AUTH_HELPER_NOT_NULL);
    ...
    private Object handleMfa(Object[] targetArgs, Object targetResult) throws Throwable {
        ...
        if (Objects.isNull(request) || Objects.isNull(response)) {
            MfaExceptionThrower.throwsMfaException(mfaProcess, new MfaException(MfaErrorEnum.NOT_SUPPORT_NON_WEB));
        }
        ...
        if (mfaProcess.bindingMfaDone(request, targetArgs, targetResult)) {
            ...
            MfaExceptionThrower.isTrue(mfaProcess, StringUtils.isNotBlank(mfaSecretKey), MfaErrorEnum.MFA_SECRET_KEY_NOT_BLANK);
        ...
        if (!mfaProcess.bindingMfa(request, targetArgs, targetResult) || mfaProcess.reBindingMfa(request, targetArgs, targetResult)) {
            ...
            MfaExceptionThrower.isTrue(mfaProcess, StringUtils.isNotBlank(authFlag), MfaErrorEnum.AUTH_FLAG_NOT_BLANK);
            MfaExceptionThrower.isTrue(mfaProcess, StringUtils.isNotBlank(authUser), MfaErrorEnum.AUTH_USER_NOT_BLANK);
        ...
        if (mfaProcess.bindingMfa(request, targetArgs, targetResult)) {
            ...
            String mfaSecretKey = mfaProcess.supplyMfaSecret(request, targetArgs, targetResult);
            MfaExceptionThrower.isTrue(mfaProcess, StringUtils.isNotBlank(mfaSecretKey), MfaErrorEnum.MFA_SECRET_KEY_NOT_BLANK_VALIDATE);
            // 宽限时间5*30秒
            ...
            if (BooleanUtils.isTrue(result)) {
                ...
                MfaExceptionThrower.throwsMfaException(mfaProcess, new MfaException(MfaErrorEnum.UNAUTHORIZED));
            }
        }
        mfaProcess.destroy(targetArgs, targetResult);
        MfaExceptionThrower.throwsMfaException(mfaProcess, new MfaException(MfaErrorEnum.UNAUTHORIZED));
    ...
    private boolean checkMfaCode(String mfaCode, String mfaSecretKey, HttpServletRequest request, Object[] targetArgs, Object targetResult) {
        ...
        try {
            ...
            MfaExceptionThrower.throwsMfaException(mfaProcess, new MfaException(MfaErrorEnum.CHECK_FAILED, e));
        } finally {
