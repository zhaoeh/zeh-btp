package zeh.btp.mfa.endpoint;

import com.google.zxing.WriterException;
import zeh.btp.mfa.MfaProcess;
import zeh.btp.mfa.aop.MfaCheckAround;
import zeh.btp.mfa.component.GoogleAuthHelper;
import zeh.btp.mfa.component.QrCodeUtils;
...
import zeh.btp.mfa.show.MfaEnum;
import zeh.btp.mfa.show.MfaInfo;
...
public class MfaEndPoint {
    ...
    public Object getMfaQrcode(HttpServletRequest request) {
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            MfaExceptionThrower.throwsMfaException(process, new MfaException(MfaErrorEnum.GENERATE_QR_CODE_FAILED, e));
        }
        if (StringUtils.isEmpty(qrStr)) {
            MfaExceptionThrower.throwsMfaException(process, new MfaException(MfaErrorEnum.QR_CODE_IS_BLANK));
        }
    ...
    public Object updateMfaBindingStatus(HttpServletRequest request) {
        MfaContext mfaContext = (MfaContext) request.getAttribute(MfaCheckAround.MFA_CONTEXT);
        MfaProcess process = mfaContext.getProcess();
        MfaExceptionThrower.notNull(process, mfaContext, MfaErrorEnum.MFA_NOT_NULL);
        if (Objects.nonNull(process)) {
            try {
                ...
                MfaExceptionThrower.throwsMfaException(process, new MfaException(MfaErrorEnum.CONFIRM_BINDING_FAILED, e));
            }
        }
