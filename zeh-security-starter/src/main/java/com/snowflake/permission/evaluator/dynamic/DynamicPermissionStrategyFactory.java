 ...
 */
@Slf4j
public class DynamicPermissionStrategyFactory<T> extends CustomizedPermissionStrategyFactorySupport<T> implements PermissionEvaluator<T> {

    public DynamicPermissionStrategyFactory(ObjectProvider<PermissionRulesConfigure<T>> permissionRulesConfigures) {
        super(permissionRulesConfigures);
    }

    @Override
    public boolean checkHasDynamicPermissions(String permission, T login, Object[] params) {


        if (CollectionUtils.isEmpty(dynamicPermissionMap)) {
            // 调度了 @cp 但是没有提供规则，默认放行
            throw new IllegalArgumentException("使用权限注解 @PreAuthorize 但DynamicPermission未提供动态权限规则");
        }
        if (StringUtils.isBlank(permission)) {
            throw new IllegalArgumentException("使用权限注解 @PreAuthorize 但hasDynamicPermissions未传递权限标志");
        }
        DynamicPermission<T> dynamicPermission = dynamicPermissionMap.get(permission);
        if (Objects.isNull(dynamicPermission)) {
            throw new IllegalStateException("使用权限注解 @PreAuthorize 权限标志未匹配到规则器");
        }
        PermissionDesc permissionResult = dynamicPermission.hasPermissions(login, params);
